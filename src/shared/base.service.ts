...
export abstract class BaseService<T extends Typegoose> {
    ...
    async push(id: string, item: InstanceType<T>): Promise<InstanceType<T>> {
        return this._model.findByIdAndUpdate(this.toObjectId(id), item, { upsert: true }).exec();
    }

    async clearArray(id: string, filter = {}): Promise<InstanceType<T>> {
        return this._model.findByIdAndUpdate(this.toObjectId(id), filter, { upsert: true }).exec();
    }

    async addArray(id: string, filter = {}): Promise<InstanceType<T>> {
        return this._model.findByIdAndUpdate(this.toObjectId(id), filter, { upsert: true }).exec();
    }
    ...
    async countSum(filter = {}): Promise<InstanceType<T>> {
        return this._model.aggregate([filter]).exec();
    }
